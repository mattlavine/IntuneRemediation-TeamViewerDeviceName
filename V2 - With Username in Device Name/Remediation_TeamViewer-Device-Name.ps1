########################################################################
# Remediation Script for TeamViewer Device Name Check
...
$logFileName = "TeamViewer_Device_Name_Remediation_v2.log"
...
if ($currentTVDeviceName -eq $desiredTVDeviceName) {
    ...
    Write-Host ""
    Write-Host "[NOTE] It's worth noting that under normal conditions the remediation script shouldn't come back saying '[NO ACTION NEEDED]' because the remediation script is only triggered by Intune if the Detection script reported '[ACTION NEEDED]' (by exiting the detection script with an error code of 1)."
    Write-Host ""
    Write-Host "[INVESTIGATION NEEDED] An investigation should be done into why the detection script reported an issue but the remediation script didn't."
...
Write-Host ""
Write-Host "[Part 4] Updating the Device Name of the device in TeamViewer..."

$tvDesiredDeviceNamePutBody = @{
    name = $desiredTVDeviceName
} 

$tvDesiredDeviceNameJSONPutBody = $tvDesiredDeviceNamePutBody | ConvertTo-Json

try {
    # POSSIBLE TO-DO: I should probably change this to use Invoke-RestMethod or swicth the others to use Invoke-WebRequest for consistancy
    # Update device with current desiredTVDeviceName using TeamViewer API
    #$tvUpdateDeviceNameResponse = Invoke-WebRequest -Uri $tvURL -UseBasicParsing -Method PUT -Headers $tvHeaders -Body $tvDesiredDeviceNameJSONPutBody -ContentType "application/json" -Verbose
    $tvUpdateDeviceNameResponse = Invoke-RestMethod -Method PUT -Uri $tvURL -Headers $tvHeaders -Body $tvDesiredDeviceNameJSONPutBody -ContentType "application/json" -Verbose
} catch {
    Write-Host ""
    Write-Host "[ERROR] Encountered an error when attempting to update the device name for '$TeamViewerManagementID'."
    Handle-APIRequestError -Error $_
    Stop-Transcript -ErrorAction SilentlyContinue
    exit 1
}

#$tvUpdateDeviceNameResponseStatusCode = $tvUpdateDeviceNameResponse.StatusCode

# This is just an extra precaution in case something strange and unexpected happens. I wanted to make sure the status code and content was captured.
#if ($tvUpdateDeviceNameResponse.StatusCode -ne 204) {
    #Write-Host "[ERROR] Unexpected status code was given when attempting to rename the device in TeamViewer."
    #Write-Host "[INFO] Status Code: " $tvUpdateDeviceNameResponse.StatusCode
    #$TVRenameUnexpectedResponse = $tvUpdateDeviceNameResponse.Content | ConvertFrom-Json | ConvertTo-Json -Depth 10
    #Write-Host "[DEBUG] Unexpected Response:"
    #Write-Host $TVRenameUnexpectedResponse
    
    #Stop-Transcript -ErrorAction SilentlyContinue
    #exit 1
#}

Write-Host "[SUCCESS] TeamViewer device has been renamed successfully."

...
$logPath += "\TV_Name_Remediation_v1.log"
...
if ($null -ne $TeamViewerManagementID) {
    ...
    if ($response.StatusCode -eq 200) {
        ...
        if ($TVDeviceName -eq $ComputerName) { #Check if names match
            ...
            $json_put = $TVNewDeviceName | ConvertTo-Json
            #Update device with current ComputerName using TeamViewer API devices endpoint
            $Result = Invoke-WebRequest -Uri $TVURL -UseBasicParsing -Method PUT -Headers $header -Body $json_put -ContentType "application/json"
            if ($Result.StatusCode -eq 204) {
                Write-Host "Device renamed successfully."
                $TVRename = $Result.Content | ConvertFrom-Json
            } else {
                Write-Host "Failed to rename device."
                $TVRename = $Result.Content | ConvertFrom-Json
                Write-Host "Rename Attempt Results: [$TVRename]"
